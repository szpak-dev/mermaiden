...
class TestRailroadIncrementalCommands:
    ...
    def test_rejects_duplicate_rules_and_unknown_expression_parents(self) -> None:
        application = Application.create()
        diagram = application.create_diagram("railroad-ebnf-beta")
        application.apply(diagram, DiagramCommand("add_rule", {"id": "rule", "label": "rule"}))

        with pytest.raises(RuntimeError, match=r"already exists"):
            application.apply(diagram, DiagramCommand("add_rule", {"id": "rule", "label": "again"}))
        with pytest.raises(RuntimeError, match=r"unknown|does not exist"):
            application.apply(
                diagram,
                DiagramCommand("add_special", {"id": "value", "label": "value", "parent_id": "missing"}),
            )
